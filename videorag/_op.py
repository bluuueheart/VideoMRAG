...
from .iterative_refinement import refine_context
from ._videoutil.refinement_utils import extract_ocr_text_for_segments, detect_objects_for_segments_owlvit, extract_keyword_queries_from_query
import numpy as np
...
def get_chunks(new_videos, chunk_func=chunking_by_video_segments, **chunk_func_params):
    ...
    for video_name in new_videos_list:
        ...
        for chunk in chunks:
            # Derive start/end time range from involved segments if available
            try:
                seg_ids = chunk.get("video_segment_id", [])
                times = []
                for s_id in seg_ids:
                    vid = s_id.rsplit('_', 1)[0]
                    idx = s_id.rsplit('_', 1)[1]
                    if vid in new_videos and idx in new_videos[vid]:
                        t_str = new_videos[vid][idx].get("time")
                        if t_str:
                            s, e = t_str.split('-')
                            times.append((float(s), float(e)))
                if len(times):
                    chunk["start_time"] = min(t[0] for t in times)
                    chunk["end_time"] = max(t[1] for t in times)
            except Exception:
                pass
...
async def videorag_query(
    ...
    # ==================================================================================
    # == Iterative Refinement Module Integration
    # ==================================================================================
    initial_context_for_refiner = []
    for s_id in retrieved_segments:
        video_name = '_'.join(s_id.split('_')[:-1])
        index_str = s_id.split('_')[-1]
        segment_data = video_segments._data.get(video_name, {}).get(index_str, {})
        # fill start/end from "time" if missing
        start_time = segment_data.get("start_time")
        end_time = segment_data.get("end_time")
        if (start_time is None or end_time is None) and "time" in segment_data:
            try:
                t0, t1 = segment_data["time"].split('-')
                start_time = float(t0)
                end_time = float(t1)
            except Exception:
                pass
        initial_context_for_refiner.append({
            "id": s_id,
            "summary": segment_data.get("content", "No summary available."),
            "start_time": start_time,
            "end_time": end_time
        })

    # Pass necessary resources to the refiner
    refiner_resources = {
        "video_path_db": video_path_db,
        "video_segments": video_segments
    }

    plan = await refine_context(
        query=query,
        initial_context=initial_context_for_refiner,
        config=global_config,
        resources=refiner_resources
    )

    refined_num_frames = global_config['fine_num_frames_per_segment']
    final_segments = list(retrieved_segments)

    # Force refine for test when requested
    if getattr(query_param, 'force_refine_test', False):
        plan = {
            "status": "refine",
            "refinement_params": {
                "new_sampling_rate_per_30s": 30,
                "temporal_expansion_needed": getattr(query_param, 'force_temporal_expansion', True),
                "run_ocr": getattr(query_param, 'force_run_ocr', True),
                "run_det": getattr(query_param, 'force_run_det', True),
                "score": 0
            }
        }

    if plan.get('status') == 'refine':
        params = plan["refinement_params"]
        refined_num_frames = int(params.get('new_sampling_rate_per_30s', refined_num_frames))
        
        segments_for_processing = list(retrieved_segments)
        if params.get("temporal_expansion_needed"):
            expanded = set(segments_for_processing)
            for s_id in list(segments_for_processing):
                vid = '_'.join(s_id.split('_')[:-1])
                idx_str = s_id.split('_')[-1]
                try:
                    idx_int = int(idx_str)
                    for delta in (-1, 1):
                        n_idx = idx_int + delta
                        n_id = f"{vid}_{n_idx}"
                        if vid in video_segments._data and str(n_idx) in video_segments._data[vid]:
                            expanded.add(n_id)
                except ValueError:
                    continue
            segments_for_processing = sorted(list(expanded), key=lambda x: (x.rsplit('_', 1)[0], int(x.rsplit('_', 1)[1])))
            print(f"[Refine-Temporal] expanded_from={len(retrieved_segments)} to={len(segments_for_processing)}")

        results = plan.get("refinement_results", {})
        ocr_map = results.get("ocr_text_map", {})
        det_map = results.get("det_text_map", {})

        if ocr_map or det_map:
            # Re-rank based on new text
            new_texts_for_rerank = []
            segment_map = {}
            for s_id in segments_for_processing:
                video_name, index_str = s_id.rsplit('_', 1)
                base_text = video_segments._data[video_name][index_str].get("transcript", "")
                ocr_text = ocr_map.get(s_id, "")
                det_text = det_map.get(s_id, "")
                combined_text = f"{base_text} [OCR] {ocr_text} [DET] {det_text}".strip()
                new_texts_for_rerank.append(combined_text)
                segment_map[combined_text] = s_id
            
            # Use the text embedding function bound to chunks_vdb (already initialized)
            embedding_func = chunks_vdb.embedding_func
            query_vec = (await embedding_func([query]))[0]
            content_vecs = await embedding_func(new_texts_for_rerank)
            
            # Compute cosine similarity
            similarities = np.dot(content_vecs, query_vec) / (np.linalg.norm(content_vecs, axis=1) * np.linalg.norm(query_vec))
            
            # Get top-k indices from re-ranking (e.g., top 5)
            top_k_reranked = min(5, len(new_texts_for_rerank))
            reranked_indices = np.argsort(similarities)[-top_k_reranked:][::-1]
            
            final_segments = [segment_map[new_texts_for_rerank[i]] for i in reranked_indices]
            # Keep a small portion of original segments to improve robustness
            keep_original = [s for s in segments_for_processing if s not in final_segments][:max(0, 2 - len(final_segments))]
            final_segments = final_segments + keep_original
            print(f"Re-ranked and selected Top-{top_k_reranked} segments: {final_segments}")
        else:
            final_segments = segments_for_processing
    
    # Use the final list of segments for captioning
    retrieved_segments = final_segments
    # ==================================================================================
    ...
    caption_results = retrieved_segment_caption(
        ...
        video_path_db,
        video_segments,
        num_sampled_frames=refined_num_frames,
        extra_text_map=None # No longer needed as info is used for re-ranking
    )
    ...
    for s_id in caption_results:
        video_name = '_'.join(s_id.split('_')[:-1])
        index = s_id.split('_')[-1]
        t0, t1 = video_segments._data[video_name][index]["time"].split('-')
        start_time_f = float(t0)
        end_time_f = float(t1)
        def _fmt(t: float):
            h = int(t // 3600)
            m = int((t % 3600) // 60)
        ...
        start_time = _fmt(start_time_f)
        end_time = _fmt(end_time_f)
