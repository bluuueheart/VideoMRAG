...
class NanoVectorDBVideoSegmentStorage(BaseVectorStorage):
    ...
    async def upsert(self, video_name, segment_index2name, segment_times_info, video_output_format):
        ...
        for index in index_list:
            segment_time_info = segment_times_info.get(index, {})
            # Align timestamp metadata from split output
            start_time = segment_time_info.get("start")
            end_time = segment_time_info.get("end")
            if (start_time is None or end_time is None) and "timestamp" in segment_time_info:
                try:
                    start_time, end_time = segment_time_info["timestamp"]
                except Exception:
                    start_time = segment_time_info.get("start")
                    end_time = segment_time_info.get("end")
            list_data.append({
